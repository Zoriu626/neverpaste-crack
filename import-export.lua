...
local base64 = { } do
    ...
    local char, byte, gsub, sub, format, concat, tostring, error, pairs =
        string.char, string.byte, string.gsub, string.sub, string.format, table.concat, tostring, error, pairs
    ...
    -- FIXED: защита от nil значений в decoder + пустой строки
    local function decode(b64, decoder)
        ...
        for i = 1, padding > 0 and n-4 or n, 4 do
            ...
            -- FIXED: защита от nil — используем 'or 0' для каждого значения
            local da = decoder[a] or 0
            local db = decoder[b] or 0
            local dc = decoder[c] or 0
            local dd = decoder[d] or 0

            local v0 = a*0x1000000 + b*0x10000 + c*0x100 + (d or 0)
            local s = cache[v0]
            if not s then
...
local file = { } do
    ...
    local function delete(path)
        ...
        local ok = ffi.C.DeleteFileA(path)
        if ok == 0 then
...
local config_system do
    ...
    menu.elements.config.actions.create:set_callback(function()
        local name = helpers.sanitize(menu.elements.config.name:get())
        if name ~= '' and not file.exists(string.format("%s\\%s", file.MASTER_PATH, name)) then
            file.write(string.format("%s\\%s", file.MASTER_PATH, name), empty_cfg)
        end
    ...
    -- LOAD  (FIXED: pcall + nil-guard + tab nil-guard)
    menu.elements.config.actions.load:set_callback(function()
        local ok, err = pcall(function()
            local name = helpers.sanitize(menu.elements.config.name:get())
            if name == '' then return end

            local cfg = file.read(string.format("%s\\%s", file.MASTER_PATH, name))
            if not cfg or cfg == '' then return end

            local data = json.parse(base64.decode(cfg))
            for k, v in pairs(data) do
                local tab = tabs[k]
                if tab then tab:import(v) end
            end
        end)
        if not ok then print('[Config] Load error: ' .. tostring(err)) end
    end)
    ...
    menu.elements.config.actions.save:set_callback(function()
        local ok, err = pcall(function()
            ...
            file.write(string.format("%s\\%s", file.MASTER_PATH, name), base64.encode(json.stringify(data)))
        end)
        if not ok then print('[Config] Save error: ' .. tostring(err)) end
    end)

    -- IMPORT  (FIXED: pcall + nil/empty-guard)
    menu.elements.tabs.actions.import:set_callback(function()
        local ok, err = pcall(function()
            ...
            for k, v in pairs(data) do
                ...
                if elements then
                    for i = 1, #funny do
                        local selected = menu.elements.tabs.tab_list:get()
                        if selected[i] and funny[selected[i]] == k then
                            elements:import(v)
                        end
                    end
                end
        ...
        if not ok then print('[Config] Import error: ' .. tostring(err)) end
    end)
    ...
    menu.elements.tabs.actions.export:set_callback(function()
        local ok, err = pcall(function()
            ...
            for i = 1, #funny do
                if selected[i] then
                    local tab_name = funny[selected[i]]
                    if tab_name then
                        data[tab_name] = tabs[tab_name]:export()
                    end
                end
            end
        ...
        if not ok then print('[Config] Export error: ' .. tostring(err)) end
    end)
end
